...
let b0 = new Variable("b0", undefined, undefined,true, false, ["never"], undefined);
let h0 = new Variable("h0", undefined, undefined,true, false, ["never"], undefined);
let d = new Variable("d", undefined, undefined,true, false, ["h"], () => {return 0.9*h.value;});
let d1 = new Variable("d1", undefined, undefined,true, false, ["h"], () => {return 0.1*h.value});
...
let As2 = new Variable("As2", undefined, undefined,false, false, ["Mu","Mt", "u1"], () => {
    ...
    if(u_r <= Uab || u_r <= u1.value){
        a = 1.25*(1-Math.sqrt(1-2*u_r));
        z = d.value*(1-0.4*a);
        Asc = 0;
    ...
    if(u.value <= Uab || u.value <= u1.value){
        ...
        z = d.value*(1-0.4*a);
        Asc = 0;
        return (Mu.value-Md.value)/(z*fsu);
    }else{
        let x1 = a1.value*d.value; 
        if(0.8*x1<h0.value){
          let u_r = Mu.value/(b.value*d.value*d.value*fbu);
          if(u_r <= Uab || u_r <= u1.value){
              a = 1.25*(1-Math.sqrt(1-2*u_r));
...
let As1 = new Variable("As1", undefined, undefined,false, false, ["Md","d","h0","fe"], () => {
  if(Mu.value <= Mt.value){
    ...
    let fsu = fe.value/1.15; 
    return Md.value/((d.value-h0.value/2)*fsu);
  }
});
...
function details(){
    document.getElementById("Asmin-detail").innerText = Math.round(Asmin*1000000000)/100000;
    document.getElementById("Asmax-detail").innerText = Math.round(Asmax*1000000000)/100000;
    document.getElementById("As1-detail").innerText = Math.round(As1.value*1000000000)/100000;
    document.getElementById("As2-detail").innerText = Math.round(As2.value*1000000000)/100000;
    document.getElementById("z-detail").innerText = Math.round(z*10000000)/100000;
    document.getElementById("xan-detail").innerText = Math.round(x*10000000)/100000;
    ...
    document.getElementById("a-detail").innerText = Math.round(a*1000000)/1000000;
    document.getElementById("a1-detail").innerText = Math.round(a1.value*1000000)/1000000;
...
function verifyELS(){
    ...
    I = b.value*Math.pow(x,3)/3 - (b.value-b0.value)*Math.pow(x-h0.value,3)/3 + 15*asc*(x-d1.value)*(x-d1.value) + 15 * As.value*(d.value-x)*(d.value-x);
    sigmaB = Ms.value*x/I;
